 ...
 * @param[in] num_coefficients_to_fit How many shape-coefficients to fit (all others will stay 0). Should be bigger than zero, or std::experimental::nullopt to fit all coefficients.
...
inline Eigen::VectorXf fit_shape(Eigen::Matrix<float, 3, 4> affine_camera_matrix,
                                 ...
                                 std::experimental::optional<int> num_coefficients_to_fit,
...
inline Eigen::VectorXf fit_shape(Eigen::Matrix<float, 3, 4> affine_camera_matrix,
                                 ...
                                 std::experimental::optional<int> num_coefficients_to_fit = std::experimental::optional<int>())
{
 ...
 * the last iteration.
 *
 * Use render::Mesh fit_shape_and_pose(const morphablemodel::MorphableModel&, const std::vector<morphablemodel::Blendshape>&, const core::LandmarkCollection<cv::Vec2f>&, const core::LandmarkMapper&, int, int, const morphablemodel::EdgeTopology&, const fitting::ContourLandmarks&, const fitting::ModelContour&, int, std::experimental::optional<int>, float).
 ...
 * @param[in] num_shape_coefficients_to_fit How many shape-coefficients to fit (all others will stay 0). Should be bigger than zero, or std::experimental::nullopt to fit all coefficients.
 * @param[in] lambda Regularisation parameter of the PCA shape fitting.
...
inline std::pair<core::Mesh, fitting::RenderingParameters> fit_shape_and_pose(
    ...
    int num_iterations,
    std::experimental::optional<int> num_shape_coefficients_to_fit,
    float lambda,
    std::experimental::optional<fitting::RenderingParameters> initial_rendering_params,
    ...
    std::vector<Eigen::Vector2f>& fitted_image_points)
{
    ...
    {
        pca_shape_coefficients.resize(*num_shape_coefficients_to_fit.operator->());
    }
    ...
    VectorXf current_combined_shape =
        current_pca_shape + blendshapes_as_basis * Eigen::Map<const VectorXf>(blendshape_coefficients.data(), blendshape_coefficients.size());
    ...
    for (int i = 0; i < landmarks.size(); ++i)
    {
        std::string name = landmarks[i].name;

        auto converted_name = landmark_mapper.convert(name);
        if (!converted_name)
        { // no mapping defined for the current landmark
            continue;
        }
        int vertex_idx = std::stoi(*converted_name.operator->());
        Vector4f vertex(current_mesh.vertices[vertex_idx][0],
                        current_mesh.vertices[vertex_idx][1],
    ...
    blendshape_coefficients = fitting::fit_blendshapes_to_landmarks_nnls(
    ...
    current_combined_shape =
        ...
        morphablemodel::to_matrix(blendshapes) *
            Eigen::Map<const VectorXf>(blendshape_coefficients.data(), blendshape_coefficients.size());
 ...
 * std::pair<render::Mesh, fitting::RenderingParameters> fit_shape_and_pose(const morphablemodel::MorphableModel&, const std::vector<morphablemodel::Blendshape>&, const core::LandmarkCollection<cv::Vec2f>&, const core::LandmarkMapper&, int, int, const morphablemodel::EdgeTopology&, const fitting::ContourLandmarks&, const fitting::ModelContour&, int, std::experimental::optional<int>, float, std::experimental::optional<fitting::RenderingParameters>, std::vector<float>&, std::vector<float>&, std::vector<cv::Vec2f>&)
 *
 * Todo: Add a convergence criterion.
 *
 * \p num_iterations: Results are good for even a single iteration. For single-image fitting and
...
fit_shape_and_pose(const morphablemodel::MorphableModel& morphable_model,
                   ...
                   std::experimental::optional<int> num_shape_coefficients_to_fit = std::experimental::nullopt, float lambda = 50.0f)
{
    ...
    return fit_shape_and_pose(morphable_model, blendshapes, landmarks, landmark_mapper, image_width,
                              ...
                              num_shape_coefficients_to_fit, lambda, std::experimental::nullopt, pca_coeffs,
