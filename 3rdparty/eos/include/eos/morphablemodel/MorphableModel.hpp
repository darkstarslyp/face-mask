    ...
    if (color_instance.size() > 0)
    {
        ...
        for (auto i = 0; i < num_vertices; ++i)
        {
            mesh.colors[i] = Eigen::Vector3f(
                std::min(std::max(color_instance(i * 3 + 0), 0.0f), 1.0f),
                std::min(std::max(color_instance(i * 3 + 1), 0.0f), 1.0f),
                std::min(std::max(color_instance(i * 3 + 2), 0.0f), 1.0f)); // We use RGB order everywhere.
        }
    }
