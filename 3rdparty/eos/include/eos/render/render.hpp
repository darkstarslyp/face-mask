...
render(core::Mesh mesh, glm::tmat4x4<float> model_view_matrix, glm::tmat4x4<float> projection_matrix,
       int viewport_width, int viewport_height, const std::experimental::optional<Texture>& texture = std::experimental::nullopt,
       ...
       bool enable_far_clipping = true)
{
    ...
    for (const auto& tri_indices : mesh.tvi)
    {
        ...
        if ((visibility_bits[0] | visibility_bits[1] | visibility_bits[2]) == 0)
        {
            std::experimental::optional<detail::TriangleToRasterize> t = detail::process_prospective_tri(
        ...
        if (vertices.size() >= 3)
        {
            ...
            {
                std::experimental::optional<detail::TriangleToRasterize> t =
